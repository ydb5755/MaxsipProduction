...
async function getTotal(agent_or_master_agent, day_request_offset){
  const result = await fetch(`/agent_reports/get_total_customer_count_for_agent_and_master_agent/${agent_or_master_agent}/${day_request_offset}`);
  const data = await result.json();
  totalTarget.innerText = data.count.toLocaleString();
}
async function getAgentsTotal(agent_or_master_agent, day_request_offset){
  const result = await fetch(`/agent_reports/get_total_number_of_agents_for_total_customer_count_by_agent_or_master_agent/${agent_or_master_agent}/${day_request_offset}`);
  const data = await result.json();
...
async function getFiveAgents(offset, agent_or_master_agent, day_request_offset){
  ...
  leftArrow.classList = '';
  const result = await fetch(`/agent_reports/get_five_agents_for_total/${offset}/${agent_or_master_agent}/${day_request_offset}`);
  const data = await result.json();
