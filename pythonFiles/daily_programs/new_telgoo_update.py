...
def processing_activation_report(file_path:str, yesterday:float, current_user_id:int, report_type:str):
    ...
    def column_mapping_telgoo_to_merged(desired_map:str) -> dict:
        ...
        if desired_map == 'telgoo_csv_to_merged':
            ...
            return 'Requested map doesnt exist, try "telgoo_csv_to_merged"'
    ...
    def get_customer_order_ids_to_insert(csv_file:str):
        ...
        with engine.connect() as conn:
            db_order_ids = conn.execute(select(Merged_Customer_Databases.c['Customer_Order_ID'])
                                            .where(Merged_Customer_Databases.c['Source_Database'] == 'Telgoo')).all()
    ...
    def csv_to_db_upsert(path_to_csv):
        ...
        telgoo_csv_to_merged_columns = column_mapping_telgoo_to_merged('telgoo_csv_to_merged')
                ...
                if indexed_ids_for_update.get(contact_order_id, None):
                    ...
                    for column, index in fields_dict.items():
                        if column in ['CREATED DATE', 'ORDER DATE', 'ACTIVATION DATE', 'DOB', 'DEACTIVATION DATE', 'BQP BIRTH DATE', 'DEVICE REIMBURSEMENT  DATE']:
                            if contact[index]:
                                whole_row_columns_and_data[telgoo_csv_to_merged_columns[column]] = parse(contact[index]).timestamp()
                            else:
                                whole_row_columns_and_data[telgoo_csv_to_merged_columns[column]] = ''
                            ...
                            if contact[index]:
                                ...
                                if digits == 6:
                                    whole_row_columns_and_data[telgoo_csv_to_merged_columns[column]] = temp_data[1:-1]
                                elif digits == 4:
                                    whole_row_columns_and_data[telgoo_csv_to_merged_columns[column]] = temp_data
                                elif digits == 3:
                                    whole_row_columns_and_data[telgoo_csv_to_merged_columns[column]] = f'0{temp_data}'
                                elif digits == 2:
                                    whole_row_columns_and_data[telgoo_csv_to_merged_columns[column]] = f'00{temp_data}'
                                elif digits == 1:
                                    whole_row_columns_and_data[telgoo_csv_to_merged_columns[column]] = f'000{temp_data}'
                                else:
                                    whole_row_columns_and_data[telgoo_csv_to_merged_columns[column]] = temp_data
                            else:
                                whole_row_columns_and_data[telgoo_csv_to_merged_columns[column]] = ''
                            ...
                            if contact[index]:
                                whole_row_columns_and_data[telgoo_csv_to_merged_columns[column]] = contact[index]
                            else:
                                whole_row_columns_and_data[telgoo_csv_to_merged_columns[column]] = ''
                    ...
                    whole_row_columns_and_data = {}
                    whole_row_columns_and_data['Source_Database'] = 'Telgoo'
                    for column, index in fields_dict.items():
                        if column in ['CREATED DATE', 'ORDER DATE', 'ACTIVATION DATE', 'DOB', 'DEACTIVATION DATE', 'BQP BIRTH DATE', 'DEVICE REIMBURSEMENT  DATE']:
                            if contact[index]:
                                whole_row_columns_and_data[telgoo_csv_to_merged_columns[column]] = parse(contact[index]).timestamp()
    ...
    def get_merged_db_telgoo_contacts_with_col_value(merged_col:str) -> dict:
        with engine.connect() as conn:
        ...
        merged_db_telgoo_contacts_with_col_value = {}
        for contact in merged_db_contacts:
            merged_db_telgoo_contacts_with_col_value[contact[order_id_index]] = contact[merged_col_index]
        
        return merged_db_telgoo_contacts_with_col_value

    def get_csv_telgoo_contacts(csv_file:str, telgoo_col:str) -> dict:
        csv_telgoo_contacts = {}
        with open(csv_file, encoding='utf-8') as csvfile:
            csvreader = csv.reader(csvfile)
            fields = next(csvreader)
            order_id_index = fields.index('ENROLLMENT ID')
            current_selected_telgoo_col_index = fields.index(telgoo_col)
            for csv_reader_contact in csvreader:
                if len(csv_reader_contact) == len(fields):
                    csv_telgoo_contacts[csv_reader_contact[order_id_index]] = csv_reader_contact[current_selected_telgoo_col_index]
        return csv_telgoo_contacts
    ...
    def get_changes(then, csv_file) -> dict:
        ...
        telgoo_to_merged_columns = column_mapping_telgoo_to_merged('telgoo_csv_to_merged')

        for telgoo_col, merged_col in telgoo_to_merged_columns.items():
            print(f'Time passed: {datetime.now()-then}. Starting {merged_col}')
            
            merged_db_telgoo_contacts_with_col_value = get_merged_db_telgoo_contacts_with_col_value(merged_col)
            csv_telgoo_contacts = get_csv_telgoo_contacts(csv_file, telgoo_col)

            for order_id, col_value in csv_telgoo_contacts.items():
                if merged_db_telgoo_contacts_with_col_value.get(order_id, None):
                    ...
                    if merged_db_telgoo_contacts_with_col_value[order_id] != col_value:
                        changes[merged_col][order_id] = {
                            ...
                            'New_Value':col_value,
                            'Old_Value':merged_db_telgoo_contacts_with_col_value[order_id],
                            'Change_Date':yesterday,
                            'Activation_Date':'',
                            'CLEC':'',
                            'Plan':'',
...
def run_file():
    ...
    maxsip_zip_file_holder = get_files_from_sftp()
    for file in os.listdir(maxsip_zip_file_holder):
        print(file)
        file_path = f'{maxsip_zip_file_holder}/{file}'
        result = processing_activation_report(file_path, yesterday_timestamp, 1, 'Telgoo Activation Report')
        print(result)
        os.remove(file_path)
    os.rmdir(maxsip_zip_file_holder)
